...
- **自动**：**新建成**的 worktree 首次打开 → 弹窗选来源与同步项 → 应用。勾「记住本次选择」后，该项目之后静默应用。
- **只在新建 worktree 时弹**：判定条件是「worktree 创建时间晚于插件安装时间」，且该项目没被问过。
  已有 worktree 一律不弹；同一个项目也只问一次（**点取消也算问过**，不再重复打扰）。
- **手动**：`Tools → Sync Worktree Settings...`（普通项目也能用，靠「浏览...」选来源；不受上面的门槛限制）。
- **反悔**：`Tools → Reset Worktree Sync Choice` 清除本项目的选择与「已询问」标记，重新打开项目即可再次弹窗。
